...
object UpstreamErrorMapper extends Logging {
  ...
  def mapToErrorResponse(err: UpstreamErrorResponse): ErrorResponse = {
    logger.warn(s"[UpstreamErrorMapper][mapToErrorResponse] Received upstream error: status=${err.statusCode}, message='${err.message}'")
    err match {
      case UpstreamErrorResponse(_, UNAUTHORIZED, _, _) =>
        logger.info("[UpstreamErrorMapper][mapToErrorResponse] Mapping 401 to Unauthorised")
        UnauthorisedErr
      case UpstreamErrorResponse(message, UNPROCESSABLE_ENTITY, _, _) =>
        val code = extractCode(message)
        code match {
          case Some("NO_SUBMISSION_DATA") =>
            logger.warn("[UpstreamErrorMapper][mapToErrorResponse] Mapping 422 (NO_SUBMISSION_DATA) to MonthlyReturnNotSubmitted")
            MonthlyReturnNotSubmitted
          case _ =>
            logger.error(
              s"[UpstreamErrorMapper][mapToErrorResponse] Unhandled 422 error code: ${code.getOrElse("none")}, mapping to InternalServerError"
            )
            InternalServerErr()
        }
      case UpstreamErrorResponse(_, INTERNAL_SERVER_ERROR, _, _) | UpstreamErrorResponse(_, BAD_GATEWAY, _, _) |
        ...
        logger.error(s"[UpstreamErrorMapper][mapToErrorResponse] Mapping ${err.statusCode} to InternalServerError")
        InternalServerErr()

      case UpstreamErrorResponse(_, statusCode, _, _) if statusCode >= BAD_REQUEST =>
        logger.error(s"[UpstreamErrorMapper][mapToErrorResponse] Unhandled upstream error with status=$statusCode, mapping to InternalServerError")
        InternalServerErr()

      case _ =>
        logger.error(s"[UpstreamErrorMapper][mapToErrorResponse] Unhandled upstream error, mapping to InternalServerError")
        InternalServerErr()
    }
  }

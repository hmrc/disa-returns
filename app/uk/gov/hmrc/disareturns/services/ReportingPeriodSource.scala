...
trait ReportingPeriodSource {
  def get(zReference: String)(implicit hc: HeaderCarrier): Future[ReportingPeriod]
}

@Singleton
class SystemReportingPeriodSource @Inject() (
  clock:                  SystemClock,
  ...
  override def get(zReference: String)(implicit hc: HeaderCarrier): Future[ReportingPeriod] =
    Future.successful(reportingPeriodService.previousMonthPeriod(clock.currentDate))
}
